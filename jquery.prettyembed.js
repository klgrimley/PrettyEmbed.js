...
(function ($, undefined) {
	$.fn.prettyEmbed = function (options) {
		...
		elem.each(function() {
			...
			// video or playlist
			if ( $(this).attr('data-pe-playlist') !== undefined ) { thisPlaylist = $(this).attr('data-pe-playlist'); }
			else if ( options.playlist !== undefined ) { thisPlaylist = options.playlist; }
			else { thisPlaylist = undefined; }
			...
			if ( $(this).is('a') ) {
				...
				$newDOMElement.attr('data-pe-videoid', thisVideoID)
							  ...
							  .attr('data-pe-playlist', thisPlaylist);
			...
			if ( thisCustomPreviewImage !== undefined && thisCustomPreviewImage !== '' ) {
				$(this).html('<img src="' + thisCustomPreviewImage + '" width="100%" alt="YouTube Video Preview" /><div class="playButton"><div class="arrow"></div></div>');
			}
			else {
...
//			}
		...
		function clickEventRunner(obj) {
			...
			if ( ( obj.attr('data-pe-playlist') == 'true') || (options.playlist == 'true') ) {
				obj.html('<iframe width="' + wrapperWidth + '" height="' + wrapperHeight + '" ' + fullScreenFlag + 'style="border:none;" src="//www.youtube.com/embed/videoseries?list=' + thisVideoID + '"></iframe>')
